...
tools, so this can gate a CLI upgrade.

WHY THIS EXISTS (2026-08-14): the worker lane spent months claiming
"research-only BY CONSTRUCTION: no Bash in --allowedTools" - and that was
false. --allowedTools only AUTO-APPROVES in -p mode; it removes nothing. A
live job called Bash, TaskCreate and ToolSearch, and a direct drill ran
`echo`. The lane reads untrusted web pages on the box holding secrets.json
and the gamepc key.

The fix (--disallowedTools) is a DENYLIST against a list Anthropic grows on
its own schedule: that same enumeration found 33 tools including a SECOND
shell (PowerShell), Cron* persistence, and Artifact/PushNotification/
SendMessage outbound channels - several of them newer than the code that was
supposed to be excluding them. A denylist is only honest if something
measures it, so this asks the CLI what it can see and fails on anything
outside the allowlist. Run it after a `claude` upgrade, not on a timer.
"""
import json
import subprocess
import sys
...
def main():
    ...
    # line that parses and has one, else fall back to the raw text.
    answer = ""
    ...
    # the list in a sentence, and prose must not read as a finding.
    ...
    if missing:
        # Not a failure: a denial that overreaches costs research quality, so
        # say it loudly and let a human judge.
