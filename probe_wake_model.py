...
def compare(a_path, b_path):
    ...
    if a.keys() != b.keys():
        sys.exit(
            f"FAIL - different wavs ({sorted(a)} vs {sorted(b)}): "
    ...
    for name in sorted(a):
        if len(a[name]) != len(b[name]):
            sys.exit(
                f"FAIL - {name}: hop count differs ({len(a[name])} vs {len(b[name])})"
            )
        ...
        fx, fy = (
            next((i for i, v in enumerate(s) if i >= WARMUP_HOPS and v >= 0.5), None)
            for s in (x, y)
        )
        drift = None if fx is None or fy is None else abs(fx - fy)
        bad = (
            dpeak > PEAK_TOLERANCE
            or (fx is None) != (fy is None)
            or (drift is not None and drift > 1)
        )
        print(
            f"  {name:16} peak {x.max():.4f} vs {y.max():.4f} "
    ...
    if not failed:
        ...
        return 0
    print(
        f"  FAIL on {failed}. The export is not transferring; "
...
def main():
    ...
    ap.add_argument(
        ...
        action="append",
        required=False,
        help="16 kHz mono wav; repeatable. Same wav on both sides of a --compare.",
    )
    ap.add_argument(
        "--engine",
        choices=("oww", "livekit"),
    ...
    for wav in args.wav:
        ...
        print(
            f"  {Path(wav).stem:16} hops={len(s):>3}  max={max(s):.4f}"
    ...
    print(
        ...
        f"p95={sorted(all_times)[int(len(all_times) * 0.95)]:.2f}"
    )
    print(
        f"  always-on cost  {mean / 80 * 100:.1f}% of one core"
