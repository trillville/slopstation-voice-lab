...
def trace(model, pcm):
    ...
    return np.array(
        [
            float(max(model.predict(pcm[i : i + CHUNK]).values()))
...
def active_rms(x):
    ...
    return (
        float(energy[loud].mean())
        if loud is not None and loud.any()
...
def bench(model_path, positives, negatives):
    ...
    # --noise-only.
    peaks = (
        np.array([trace(m, load_wav(p)).max() for p in positives])
        if positives
        else np.zeros(0)
    )
...
def operating_point(peaks, neg_traces, hours, target_fa):
    ...
    for t in THRESHOLDS:
        ...
        rows.append((float(t), rec, int(fa), fa / hours if hours else float("nan")))
...
def main():
    ...
    ap.add_argument(
        ...
        default=None,
        metavar="DB",
        help="also measure recall with the phrase "
    ...
    if not models:
        ...
        seen = set()
        models = [
            m for m in models if not (m.resolve() in seen or seen.add(m.resolve()))
        ]
    ...
    if not models:
        sys.exit(
            f"no .onnx found (checked {len(missing)} explicit path(s), "
            f"else {root / 'artifacts'} and the repo models dir)"
        )

    print(
        f"positives  {len(positives)} utterances"
    ...
    for mp in models:
        ...
        if best and peaks.size:
            ...
            entry.update(
                threshold=t,
                recall=round(k / peaks.size, 3),
                recall_ci=[round(lo, 3), round(hi, 3)],
                fa_events=int(best[2]),
                fa_per_hour=round(best[3], 2),
            )
    ...
    if args.noise_only:
        ...
        print("-" * 60)
        for name, r in sorted(results.items(), key=lambda kv: kv[1]["noise_ceiling"]):
            print(f"{name:36}{r['noise_ceiling']:>14.3f}{r['negative_hours']:>10.2f}")
        print(
        ...
        for name, r in sorted(
            results.items(),
            key=lambda kv: (kv[1]["recall"] is not None, kv[1]["recall"] or 0),
            reverse=True,
        ):
            if r["recall"] is None:
                print(
                    f"{name:36}{r['peak_median']:>9.3f}{r['peak_min']:>9.3f}"
        ...
        print(
            ...
            "real utterance: if that sits\nbelow the threshold, some way of "
            "saying it never works, whatever the median says."
        )
        print(
    ...
    if args.snr_sweep is not None and positives:
        ...
        print(
            f"each model at its OWN threshold from the table above; "
            f"{len(positives)} utterances per cell\n"
        )
        print(f"{'model':34}{'thresh':>7}" + "".join(f"{s:>+7.0f}dB" for s in snrs))
        print("-" * (41 + 9 * len(snrs)))
        for name, r in sorted(results.items(), key=lambda kv: -(kv[1]["recall"] or 0)):
            if r["threshold"] is None:
                continue
            sw = snr_sweep(
                next(m for m in models if Path(m).stem == name),
                positives,
                pool,
                snrs,
                r["threshold"],
    ...
    out = args.json or root / (
        "bench_results.noise.json" if args.noise_only else "bench_results.json"
    )
