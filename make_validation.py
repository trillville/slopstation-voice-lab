    ...
    for path in wav_paths:
        ...
        if sr != RATE:
            sys.exit(
                ...
                f"python record_room.py, which writes 16 kHz mono."
            )
        if audio.ndim > 1:
            audio = audio[:, 0]
...
def main():
    ...
    ap.add_argument(
        "--heldout", type=Path, default=None, help="default: <root>/data/heldout"
    )
    ap.add_argument(
        "--restore", action="store_true", help="put livekit's stock validation set back"
    )
    ap.add_argument(
        "--replace",
        action="store_true",
    ...
    print(
        f"window   {cfg.augmentation.clip_duration}s = {window} samples, non-overlapping"
    )
    ...
    for chunk in clips(wavs, window):
        ...
        if len(rows) % 500 == 0:
            print(
                f"  {len(rows)} clips ({len(rows) * cfg.augmentation.clip_duration / 3600:.2f} h)",
                flush=True,
            )
    if not rows:
